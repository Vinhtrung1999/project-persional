...
Router.get('/getSessionInputPro/:idcount?', CheckGet, (req, res) =>{
    if(!req.session.username)
        return res.json({"code":3, "message":"please login"})
...
//[POST]
Router.post('/addWh', CheckLogin, (req, res) => {
    if(!req.session.username)
        return res.json({"code":3, "message":"please login"})
...
//[DELETE]
Router.delete('/deleteWH', CheckLogin, (req, res) => {
    if(!req.session.username)
        return res.json({"code":3, "message":"please login"})
...
//[UPDATE]
Router.post('/updatePriceProWH', CheckLogin, (req, res) => {
    if(!req.session.username)
        return res.json({"code":3, "message":"please login"})
